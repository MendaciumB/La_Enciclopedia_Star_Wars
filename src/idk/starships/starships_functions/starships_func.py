...
def starship_total_pages(session: Session) -> int:
    """Get the total number of pages of starships from the Star Wars API."""
    total_pages: int = 1
...
def starship_page_list(session: Session, page: int) -> list[str] | None:
    """Get a list of starship names of a page from the Star Wars API."""
...
def search_starship_by_name(session: Session, name: str) -> dict[str, str | list[str]] | None:
    ...
    if (response := get_handles_exception(session, f"{BASE_URL}starships/?search={name}")) is not None:
        ...
        starship["pilots"] = [str(search_character_name_by_url(session, pilot)) for pilot in starship["pilots"]]
        starship["films"] = [str(search_film_name_by_url(session, film)) for film in starship["films"]]
        return starship
    return None
